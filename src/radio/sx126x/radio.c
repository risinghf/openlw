...
static void SX126xLogLoraOpRecord(uint8_t op_type)
{
    SX126x_t *sx126x = &SX126x;
    LogRf_t *log_rf = RadioLogObjectGet();

    log_rf->flag = true;
    log_rf->op_type = op_type;
    log_rf->freq = sx126x->Settings.Channel;
    log_rf->sf = sx126x->ModulationParams.Params.LoRa.SpreadingFactor;
    log_rf->bw = sx126x->ModulationParams.Params.LoRa.Bandwidth + 3;
    (op_type) ? (log_rf->pream = sx126x->PacketParams.Params.LoRa.PreambleLength)
    : (log_rf->pream = sx126x->Settings.LoRa.RxSingleTimeout);
    log_rf->pow = sx126x->Settings.LoRa.Power;
}

static void SX126xLogFskOpRecord(uint8_t op_type)
{
    SX126x_t *sx126x = &SX126x;
    LogRf_t *log_rf = RadioLogObjectGet();
    ...
    log_rf->op_type = op_type;
    log_rf->freq = sx126x->Settings.Channel;
    log_rf->sf = sx126x->ModulationParams.Params.Gfsk.BitRate;
    log_rf->pream = sx126x->PacketParams.Params.Gfsk.PreambleLength;
    log_rf->bw = sx126x->ModulationParams.Params.Gfsk.Bandwidth;
    log_rf->pow = sx126x->Settings.Fsk.Power;
}

#define LOG_RF_LORA_OP(type)                               SX126xLogLoraOpRecord(type)
#define LOG_RF_FSK_OP(type)                                SX126xLogFskOpRecord(type)
#define LOG_RF_DATA_PUSH(type, buf, len, rssi, snr)       RadioLogRfDataPush(type, buf, len, rssi, snr)
                         ...
                         bool iqInverted, bool rxContinuous )
{
    ...
    switch( modem )
    {
        ...
        case MODEM_LORA:
            SX126x.Settings.LoRa.RxSingleTimeout = symbTimeout;
            SX126xSetStopRxTimerOnPreambleDetect( false );
            ...
            RadioStandby( );
            RadioSetPublicNetwork( RadioPublicNetwork.Current ); // add by yw
            RadioSetModem( ( SX126x.ModulationParams.PacketType == PACKET_TYPE_GFSK ) ? MODEM_FSK : MODEM_LORA );
            SX126xSetModulationParams( &SX126x.ModulationParams );
            SX126xSetPacketParams( &SX126x.PacketParams );
                        ...
                        uint8_t hopPeriod, bool iqInverted, uint32_t timeout )
{
    ...
    {
        case MODEM_FSK:
            SX126x.Settings.Fsk.Power = power;
            SX126x.ModulationParams.PacketType = PACKET_TYPE_GFSK;
            SX126x.ModulationParams.Params.Gfsk.BitRate = datarate;
        ...
        case MODEM_LORA:
            SX126x.Settings.LoRa.Power = power;
            SX126x.ModulationParams.PacketType = PACKET_TYPE_LORA;
            SX126x.ModulationParams.Params.LoRa.SpreadingFactor = ( RadioLoRaSpreadingFactors_t ) datarate;
            SX126x.ModulationParams.Params.LoRa.Bandwidth =  Bandwidths[bandwidth];
            SX126x.ModulationParams.Params.LoRa.CodingRate= ( RadioLoRaCodingRates_t )coderate;
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
    LOG_RF_DATA_PUSH(1, buffer, size, 0, 0);
...
void RadioStartCad( void )
{
    ...
    uint8_t cadDetPeak = 29;
    RadioLoRaCadSymbols_t symbols = LORA_CAD_01_SYMBOL;
    ...
    SX126xSetCadParams( symbols, cadDetPeak, cadDetMin, LORA_CAD_ONLY, 2000);
    SX126xSetCad( );
    LOG_RF_LORA_OP(2);
}
    ...
    if( isIrqFired == true )
    {
        ...
        if( ( irqRegs & IRQ_RX_DONE ) == IRQ_RX_DONE )
        {
            ...
            else
            {
                ...
                {
                    RadioPktStatus.Params.LoRa.SnrPkt *= 4;
                    LOG_RF_DATA_PUSH(0, RadioRxPayload, size, RadioPktStatus.Params.LoRa.RssiPkt, RadioPktStatus.Params.LoRa.SnrPkt);
